  ...
  return (
    <SyntaxHighlighter
      ...
      lineNumberStyle={(lineNumber) => {
        ...
        if (highlightLineNumbers.includes(lineNumber))
          style.background =
            "linear-gradient(to right, rgba(255, 232, 115, 1) 70%, rgba(255, 232, 115, 0))";
        return style;
      }}
      lineProps={(lineNumber) => {
        const lineIsClickable =
          clickableLines && clickableLines.includes(lineNumber);
        let style = {
          flexFlow: "wrap",
        };
        ...
        if (
          highlightLinesPrimary &&
          highlightLinesPrimary.includes(lineNumber)
        ) {
          style.background =
            "linear-gradient(to right, rgba(193, 222, 241, 0.9) 90%, rgba(221, 222, 241, 0))";
        } else if (
          highlightLinesSecondary &&
          highlightLinesSecondary.includes(lineNumber)
        ) {
          style.background =
            "linear-gradient(to right, rgba(222, 222, 222, 0.9) 90%, rgba(222, 222, 222, 0))";
        }
        return {
          style: style,
