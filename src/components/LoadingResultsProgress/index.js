...
ProgressState.resolve = (state) => {
  ...
  if (typeof state !== "string")
    throw "State must be either a string or a number";
...
const Circle = styled.circle`
  ...
  ${(props) =>
    ...
    css`
      stroke: ${props.state === ProgressState.SUCCESS
        ? colors.SUCCESS
        : colors.DANGER};
    ...
    >
      ...
      {(state === ProgressState.SUCCESS && (
        <Check
          ...
          length={side / 2}
          strokeWidth={sw}
          d={`M ${side / 2.7} ${side / 2} l ${side / 10} ${side / 11} ${
            side / 5
          } ${-side / 5}`}
        />
      )) ||
        (state === ProgressState.ERROR && (
          <Cross
            ...
            length={0.9 * side}
            strokeWidth={sw}
            d={`M ${side / 3} ${side / 3} l ${side / 3} ${side / 3} M ${
              side / 3
            } ${(2 * side) / 3} l  ${side / 3} ${-side / 3}`}
          />
        ))}
    </Wrapper>
  );
